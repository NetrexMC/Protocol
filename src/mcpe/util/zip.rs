...
pub fn zip(raw_buf: &[u8]) -> io::Result<Vec<u8>> {
    ...
    match reader.read_to_end(&mut ret_data) {
        ...
        Err(_) => {
            // try zlib
            let mut reader = ZlibDecoder::new_with_buf(raw_buf, vec![0; 256]); // 256 used to be: 2 * 1024 * 1024
            match reader.read_to_end(&mut ret_data) {
                Ok(_) => Ok(ret_data.clone()),
                Err(v) => Err(v),
            }
...
pub fn unzip(buf: &mut [u8]) -> io::Result<Vec<u8>> {
    if buf.len() < 256 {
